...
int main(int argc, char **argv)
{
    ...
    while ((seek_target = mpg123_feedseek(m, seek_point, SEEK_SET, &inoffset)) == MPG123_NEED_MORE)
    {
        fprintf(stderr, "[debug] seek_point %lld\n", (long long)seek_point);
        fprintf(stderr, " *** need more data before deciding on fuzzy input offset ***\n");

        fp = fopen("Daydream - Soobin Hoang SonThaoboy (Hiderway Remix).mp3", "r");
        fprintf(stderr, "[debug] music open\n");
        size_t len = fread(buf, 1, INBUFF, fp);
